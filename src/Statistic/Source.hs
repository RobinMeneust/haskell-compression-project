...
entropy xs = sum [-p * logBase 2 p | p <- probabilities]
  where
    ...
    totalCount = sum $ map snd $ toList counts
    probabilities = map (\(_, c) -> fromIntegral c / fromIntegral totalCount) $ toList counts
...
orderedCounts = sortBy (flip compare `on` snd) . toList . occurrences
