...
tree symbols = buildTree $ sortBy (compare `on` snd) $ orderedProbabilities symbols
...
buildTree :: Ord a => [(a, Double)] -> Maybe (EncodingTree a)
...
buildTree symbols = case (leftTree, rightTree) of
  ...
  where
    ...
    totalCount = sum $ map (\(_, cnt) -> round cnt) symbols
